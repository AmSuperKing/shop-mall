<template>
  <view class="list-wrapper">
    <view class="top-menu">
      <text v-if="!emptyList" class="top-menu-info">全部共{{count}}件商品</text>
      <text v-else class="top-menu-info">全部共0件商品</text>
      <text
        v-show="!showComplete"
        class="top-menu-btn"
        @click="handleProduct"
    ...
    <view class="have-enquiry-list" v-if="!emptyList">
      <view class="uni-list" v-for="item of items" :key="item.num">
        <checkbox-group @change="checkboxChange($event, item.num)">
          <view class="list-item-box flex-box" @click="toProductDetail">
            ...
            <view class="list-item-info">
              <view class="item-info-img-box">
                <image src="../../../static/imgs/激光测距仪.jpeg" class="item-info-img"></image>
              </view>
              <view class="item-info-text">
                <view class="item-info-desc">{{item.name}}</view>
                <view class="item-info-norms">规格：{{item.norms}}</view>
                <view class="item-info-num">商品号：{{item.num}}</view>
              </view>
            </view>
          </view>
        </checkbox-group>
    ...
    <view class="empty-enquiry-list" v-if="emptyList">
      ...
      <view class="empty-tips">询价篮暂无商品，去浏览看看吧</view>
...
export default {
  ...
  methods: {
    ...
    toProductDetail() {
      this.$Router.push('/pages/productDetail/detail')
    },
    submitDelete() {
      let _this = this
      let items = this.items
